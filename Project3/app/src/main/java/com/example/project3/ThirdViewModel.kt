    ...
    fun currentWeather(queue: RequestQueue, city: String) {
        ...
        val stringRequest = StringRequest(
            ...
            Response.Listener<String> { response ->
                ...
                val iconUrl = "https://openweathermap.org/img/w/${weather.getString("icon")}.png"
...
// Add the request to the RequestQueue.
    ...
    fun addItems(queue: RequestQueue, lon: Double, lat: Double) {
        listItems.clear()
        val url =
            "https://api.openweathermap.org/data/2.5/onecall?lat=$lat&lon=$lon&exclude=minutely,hourly,alert&units=imperial&appid=$API"
        val stringRequest =
            StringRequest(
                Request.Method.GET, url, Response.Listener<String> { response: String? ->
                    ...
                    for (i in 0..6) {
                        ...
                        var date = (SimpleDateFormat(
                            "EEE, MMMM dd",
                            Locale.ENGLISH
                        ).format(Date(toDate * 1000)))
                        var icon = wlist.getJSONObject(i).getJSONArray("weather").getJSONObject(0)
                            .getString("icon")
                        var iconURL = "https://api.openweathermap.org/img/w/$icon.png"
                        listItems.add(Items(date, daytemp, iconURL))
                    }
